...
const Message = ({message}) => {
  ...
  return (
    <div>
      ...
      :
      (
        <div className='inline-flex flex-col gap-2 p-2 px-4 max-w-2xl bg-primary/20 dark:bg-[#57317C]/30 borser border-[#80609F]/30 rounded-md my-4'>
        {message.isImage ? (
        ...
        <span className='text-xs text-gray-400 dark:text-[#B1A6C0]'>{moment(message.timetamp).fromNow()}</span>
        </div>
      )
    }
    </div>
